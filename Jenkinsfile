pipeline {
  ...
  environment {
    ...
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-nginx'
    CONTAINER_NAME = 'nginx'
    DOCKERHUB_IMAGE = 'linuxserver/nginx'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/nginx") | .uuid' || :)
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/nginx",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-nginx",\
                       "backendUrl":"https://ghcr.io/linuxserver/nginx",\
                       "publicUrl":"https://lscr.io/linuxserver/nginx"}' || :
              else
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker buildx build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/linuxserver/docker-nginx/packages\" \
          --label \"org.opencontainers.image.documentation=https://docs.linuxserver.io/images/docker-nginx\" \
          --label \"org.opencontainers.image.source=https://github.com/linuxserver/docker-nginx\" \
